...
moveIter :: Int -> Set Point -> String -> Set Point
moveIter rest starterSet moves = go starterSet Wide (2, 4) moves rest
  where
    go :: Set Point -> Shape -> Point -> String -> Int -> Set Point
    go rocks _     loc  _       0    = rocks
    go rocks shape loc (m:mvs) restNum = go nextRocks nextShape nextLoc nextMoves nextNum
      where
        mv
          | m == '<'  = L
          | m == '>'  = R
        ...
        highest = maximum $ S.map snd nextRocks
        ...
        nextRocks = if not toRest then rocks else case shape of
                   Wide -> folder wideEx
                   Tall -> folder tallEx
                   Crs -> folder crsEx
                   El -> folder elEx
...
main = do
  ...
  print $ maximum $ S.map snd $ moveIter 2022 startSet input
