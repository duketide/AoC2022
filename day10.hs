...
main = do
 ...
 let input = lines rawInput
     ...
     result = map (\x -> if abs (uncurry (-) x) < 2 then "#" else ".") posWithCycle
 print $ total 20 + total 60 + total 100 + total 140 + total 180 + total 220 
 print $ take 40 result
 print $ take 40 (drop 40 result)
 print $ take 40 (drop 80 result)
 print $ take 40 (drop 120 result)
 print $ take 40 (drop 160 result)
 print $ take 40 (drop 200 result)
